...
async def button_led_control(led: RGBLED, queue: PriorityQueue[tuple[int, dict[str, str]]], exit: Event):
    async def execute(timestamp: int, command: dict[str, str], controller: ButtonLEDController):
        ...
        elif command['value'] == "DISTANCE":
            distance = command['distance']
            # Assuming distance ranges from 0 to 10, normalize to 0-1 for brightness
            brightness = max(0, min(1, (10 - distance) / 10))
            controller.led.color = (brightness, brightness, brightness)
...
async def sensor_control(sensor, matrix: PixelStrip, queue: PriorityQueue[tuple[int, dict[str, str]]], exit: Event,maxDis: float):
    last_print_time = 0  # Initialize a variable to keep track of the last print time
    previous_distance = None  # Initialize to keep track of the previous distance
    distances_with_accuracy = []  # List to store distances along with their accuracy

    async def execute(timestamp: int, distance: float):
        command = {'type': 'SENSOR', 'value': 'DISTANCE', 'distance': distance}
    ...
    def pulse_effect(brightness: float, distance: float, maxdis: float):
        ...
        for i in range(steps):
            ...
            matrix.show()
            time.sleep(sleep_time)  # Adjust sleep time to create a distance-dependent pulse frequency
        ...
        if sensor._s.in_waiting > 0:
            ...
            if data.startswith('$JYRPO'):
                ...
                if len(parts) >= 6:
                    ...
                    await execute(timestamp, distance)  # Send maximum distance to the queue
