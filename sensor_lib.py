...
class DFRobot_mmWave_Radar:
    ...
    '''If there is JYRPO type data in the serial port returns: 
    [num of items(int), item id(int), distance(float), accuracy(float)]'''    
    def readData(self):
        if self._s.in_waiting > 0:
                data = self._s.readline().decode('utf-8').strip().replace(" ", "")
                if "JYRPO" in data and "leapMMW:/>" not in data:
                        a,b,c,d,e,f,g = [x for x in data[7:].split(',')]
